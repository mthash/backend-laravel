...
class MiningFluctuateTask extends Command
{
     ...
     * @var string
     */
    protected $signature = 'hash:fluctuate';
    ...
    public function handle()
    {
        ...
        foreach ($pools as $pool) {
            ...
            // Updating assets
            $algos = Algo::where('pool_id', $pool->id)->get();
//dd($algos);
            if ($algos) {
                foreach ($algos as $algo) {
                    $asset = Asset::where('algo_id', $algo->id)->first();
//dd($pool->total_hashrate);
                    if ($asset) {
